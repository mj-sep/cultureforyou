...
public class PickVer1Fragment extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstaceState) {
        ...
        Log.d("pick1artistlist", String.valueOf(pickartistlist));
        Log.d("pick1pickplidlistget0", pickplidlist.get(0));
        String pickfirstid = pickplidlist.get(0);
        ...
        pick_play.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("moodplaylist", pickplidlist); // 해당 추천 플리의 플레이리스트 ID 목록
                intent.putExtra("selectmood", pickmoodlist.get(0)); // 해당 추천 플리의 1번째 플리 대표감성
                intent.putExtra("select_playlist_popup", select_playlist); // 해당 추천 플리의 1번째 플리 정보
                intent.putExtra("selectplaylistid", pickplidlist.get(0)); // 선택한 플레이리스트 ID (29)
                intent.putExtra("streaming", "0"); // 재생목록 앞뒤 버튼 막히게
                intent.putExtra("picktitle", pick_title.getText().toString()); // 플리 제목 (비가 오는 날 듣기 좋은)
                intent.putExtra("moodnamelist", pickmoodlist); // 무드값 리스트
